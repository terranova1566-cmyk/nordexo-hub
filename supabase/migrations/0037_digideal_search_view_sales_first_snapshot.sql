-- DigiDeal: treat the first purchased_count snapshot as "sales so far" for new deals.
--
-- Rationale:
-- - Some products have only a single daily snapshot but non-zero purchased_count.
-- - With only one snapshot, delta-based sold_today/sold_7d would otherwise be NULL -> 0.
-- - For deal workflows, it's more useful to show purchased_count as the initial sales.
...
daily as (
  select
    ...
    case
      ...
      when daily_raw.prev_count is null then greatest(daily_raw.purchased_count, 0)
