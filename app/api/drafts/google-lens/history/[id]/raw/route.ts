...
export async function GET(
  ...
  const response = await fetch(`${toServiceUrl()}/api/google-lens/history/${encodeURIComponent(id)}/raw`, {
    method: "GET",
    cache: "no-store",
  });
  ...
  if (!response.ok) {
    const error =
      parsed && typeof parsed === "object" && "error" in parsed
        ...
        : "Google Lens raw history lookup failed.";
    return NextResponse.json({ error: error || "Google Lens raw history lookup failed.", details: parsed }, { status: response.status });
  }
