...
const buildRightFadeOverlay = (size: number, fadePercent: number) => {
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      // Linear fade: 100% white at the left edge -> 0% white at fadeWidth.
      const t = fadeWidth <= 1 ? 1 : x / (fadeWidth - 1);
      const whiteMix = Math.max(0, 1 - t);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const rightZoomedSize = Math.max(tile, Math.round(tile * RIGHT_SCENE_ZOOM));
    const rightExtractLeft = Math.max(0, rightZoomedSize - tile);
    const rightExtractTop = Math.max(0, Math.floor((rightZoomedSize - tile) / 2));

    const rightSceneBuffer = await sharp(sceneImage.absolutePath)
      .rotate()
      .resize(rightZoomedSize, rightZoomedSize, {
        fit: "cover",
        position: "centre",
      })
      .extract({
        left: rightExtractLeft,
        top: rightExtractTop,
        width: tile,
        height: tile,
      })
    ...
    const { raw: rightFadeOverlayRaw, fadeWidth } = buildRightFadeOverlay(tile, fadePercent);
    ...
    const outputBuffer = await sharp({
      ...
      .composite([
        ...
        { input: leftCutoutBuffer, left: 0, top: 0 },
      ])
    ...
    const fileBaseName = `${toSafeBaseName(spu)}-digideal-main-overlay-${buildTimestampToken()}`;
