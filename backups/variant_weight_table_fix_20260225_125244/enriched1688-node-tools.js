...
const WEIGHT_KEYWORD_RE =
  /(重量|净重|毛重|单重|克重|规格|尺寸|体积|pack|pcs?|公斤|千克|克|\b\d+(?:[.,]\d+)?\s*(?:kg|g)\b)/i;
...
function toWeightGrams(value, options = {}) {
  ...
  if (unitText.includes('kg') || unitText.includes('公斤') || unitText.includes('千克')) {
...
function normalizeNameLoose(value) {
  return normalizeNameStrict(value).replace(/[（(].*?[）)]/g, '');
}

function inferWeightScope(raw, line) {
...
function collectWeightFocusedText(value, options = {}) {
...
function extractTextWeightMentions(value, options = {}) {
...
function parseWeightCellByUnit(cell, unitHint) {
...
function parseVariantWeightTableFromReadableText(value, options = {}) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const nameIdx = Math.max(
      0,
      headers.findIndex((cell) => headerNameRe.test(cell))
    );
...
function buildVariantLabel(combo) {
...
function findTableWeightForLabel(label, weightByName) {
...
function enrichVariationsFromTable(variations, parsedTable) {
...
function collectVariantRows(variations) {
  const combos =
    variations && typeof variations === 'object' && Array.isArray(variations.combos)
      ? variations.combos
      : [];
...
function buildStructuredVariantSignal(variantRows, parsedTable) {
...
function normalizePackSignature(value) {
...
function buildWeightCandidates({ productWeights, textMentions, parsedTable, variantRows }) {
  const out = [];
  const seen = new Set();
...
function buildQualityBlock(variantRows, structuredSignal, weightsBlock) {
  const rows = Array.isArray(variantRows) ? variantRows : [];
...
function build1688Enhancements(input = {}) {
  ...
  const source = asText(input.source) || 'backend_puppeteer';
...
module.exports = {
  build1688Enhancements,
  toWeightGrams,
  extractTextWeightMentions,
